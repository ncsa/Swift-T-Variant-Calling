...
(string out) bwa (string bwadir, string index, string R1, string R2, string rgheader) "align" "0.0" [
	"set <<out>> [ bwa <<bwadir>> <<index>> <<R1>> <<R2>> <<rgheader>>]"
];

(string out) samtools_view (string samtoolsdir, int|float|string|boolean... args) "align" "0.0" "samtools" [
...
pipe (string proc1, string proc2, string out) 
	"align" "0.0" 
