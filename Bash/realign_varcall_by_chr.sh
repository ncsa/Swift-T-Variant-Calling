...
dedupsortedbam=$AlignDir/${SampleName}.wdups.sorted.bam                # name of the bam file that align-dedup produced
chrdedupsortedbam=${SampleName}.$chr.wdups.sorted.bam               # name of the aligned file -one chr out of dedupsortedbam
...
if [ ! -s $dedupsortedbam ]
then
    MSG="$dedupsortedbam aligned-dedup file not found"
    echo -e "program=$0 stopped at line=$LINENO. Reason=$MSG" | mail -s "[Task #${reportticket}]" "$redmine,$email"
    exit 1;
fi
...
set +x
echo -e "\n### split aligned.bam by region with  $dedupsortedbam and chr=$chr                       ###\n"     
set -x
cd $RealignDir

$samtoolsdir/samtools view -bu -@ $thr -h $dedupsortedbam $chr > $chrdedupsortedbam
...
if [ $exitcode -ne 0 ]
then
	 MSG="samtools command failed to split $dedupsortedbam by chr exitcode=$exitcode"
	 echo -e "program=$scriptfile stopped at line=$LINENO.\nReason=$MSG\n$LOGS"
	 exit $exitcode;
fi
if [ -s $chrdedupsortedbam ]
then  
    set +x	   
    echo -e "### the file was created. But we are not done.     #############"
    echo -e "### sometimes we may have a BAM file with NO alignmnets      ###"
    set -x
    numAlignments=$( $samtoolsdir/samtools view -c $chrdedupsortedbam ) 
    
    echo `date`
    if [ $numAlignments -eq 0 ]
    then
	echo -e "${SampleName}\tREALIGNMENT\tFAIL\tsamtools command did not produce alignments for $chrdedupsortedbam\n" >> $qcfile	    
	MSG="samtools command did not produce alignments for $chrdedupsortedbam"
	echo -e "program=$scriptfile stopped at line=$LINENO.\nReason=$MSG\n$LOGS" | mail -s "[Task #${reportticket}]" "$redmine,$email"
	exit 1;
    else
	set +x
	echo -e "####### $chrdedupsortedbam seems to be in order ###########"
   	set -x 
    fi
else 
    MSG="samtools command did not produce a file $chrdedupsortedbam"
    echo -e "program=$scriptfile stopped at line=$LINENO.\nReason=$MSG\n$LOGS" | mail -s "[Task #${reportticket}]" "$redmine,$email"        
    exit 1;          
fi	

echo -e "\n### index  $chrdedupsortedbam                                 ###\n"  

$samtoolsdir/samtools index $chrdedupsortedbam
...
if [ $exitcode -ne 0 ]
then
	 MSG="samtools index command failed to split $dedupsortedbam by chr exitcode=$exitcode"
	 echo -e "program=$scriptfile stopped at line=$LINENO.\nReason=$MSG\n$LOGS"
	 exit $exitcode;
fi
set +x
...
if [ $analysis == "VC_WITH_REALIGNMENT" ]; then 
	...
	if [ ! -s ${SampleName}.$chr.realignTargetCreator.intervals ]; 	then
       		...
       		ln -s $chrdedupsortedbam $RealignDir/$realignedbam
	else 
		set +x
		echo -e "\n\n##################################################################################" 
		...
		set -x
		$javadir/java -Xmx8g -Djava.io.tmpdir=$tmpdir -jar $gatkdir/GenomeAnalysisTK.jar \
      			-R $ref_local -I $chrdedupsortedbam -T IndelRealigner $realparms \
	...
	realignedbam=$chrdedupsortedbam	#if no realignment required, then use the dedupsortedbam file as input to the next step. This is to avoid introducing a new variable
fi
...
then
	echo -e "${SampleName}\tRECALIBRATION\tWARN\t$SampleName.$chr.recal_report.grp is an empty file. Skipping recalibration cmd\n" >> $qcfile
        ln -s $chrdedupsortedbam $RealignDir/$recalibratedbam
else
	set +x 
	echo -e "\n\n##################################################################################" 
